...
function createMRHandlers(opts = {}) {
  return {
    /**
    ...
    async mrComment({ cwd, id, body, file, line }) {
      ...
      // Skip if body already contains the signature (agent may include it)
      const signedBody = body.includes("Posted by Codebrain AI Review")
        ? body
        : body + COMMENT_SIGNATURE;

      // ── Try API token to post comment ──
      // Priority: Project Access Token (bot user) > OAuth token (personal user)
      const { provider, repo } = guard.gitInfo;
      const getBotToken = opts.getBotToken;
      const getOAuthToken = opts.getOAuthToken;
      const botToken = getBotToken ? getBotToken(provider) : null;
      const userToken = getOAuthToken ? await getOAuthToken(provider) : null;
      const apiToken = botToken || userToken;
      const via = botToken ? "bot-token" : "oauth-user";

      if (apiToken) {
        const mrUrl = provider === "gitlab"
          ? `https://gitlab.com/${repo}/-/merge_requests/${id}`
          : `https://github.com/${repo}/pull/${id}`;
        try {
          if (provider === "github") {
            if (file && line) {
              const prApiUrl = `https://api.github.com/repos/${repo}/pulls/${id}`;
              const prRes = await fetch(prApiUrl, {
                headers: { Authorization: `Bearer ${apiToken}`, Accept: "application/vnd.github.v3+json", "User-Agent": "Codebrain-ReviewBot" },
              });
              const prData = await prRes.json();
              const commitSha = prData.head?.sha;
              if (commitSha) {
                const reviewRes = await fetch(`https://api.github.com/repos/${repo}/pulls/${id}/reviews`, {
                  method: "POST",
                  headers: { Authorization: `Bearer ${apiToken}`, Accept: "application/vnd.github.v3+json", "Content-Type": "application/json", "User-Agent": "Codebrain-ReviewBot" },
                  body: JSON.stringify({ body: signedBody, commit_id: commitSha, path: file, line: parseInt(line, 10), side: "RIGHT", event: "COMMENT" }),
                });
                if (reviewRes.ok) {
                  if (opts.emitNotification) opts.emitNotification({ type: "mr_review", title: `Review inline no PR !${id}`, body: `Comentário em ${file}:${line} via ${via}`, level: "success", mr_id: parseInt(id, 10), mr_url: mrUrl, provider });
                  return { ok: true, provider, inline: true, file, line, via };
                }
              }
            }
            const commentRes = await fetch(`https://api.github.com/repos/${repo}/issues/${id}/comments`, {
              method: "POST",
              headers: { Authorization: `Bearer ${apiToken}`, Accept: "application/vnd.github.v3+json", "Content-Type": "application/json", "User-Agent": "Codebrain-ReviewBot" },
            ...
            if (commentRes.ok) {
              ...
              if (opts.emitNotification) opts.emitNotification({ type: "mr_review", title: `Review postado no PR !${id}`, body: `Comentário via GitHub ${via}`, level: "success", mr_id: parseInt(id, 10), mr_url: mrUrl, provider });
              return { ok: true, provider, comment_url: data.html_url || null, via };
            }
          }
          ...
          if (provider === "gitlab") {
            ...
            if (file && line) {
              const discRes = await fetch(`https://gitlab.com/api/v4/projects/${encodedRepo}/merge_requests/${id}/discussions`, {
                method: "POST",
              ...
              if (discRes.ok) {
                if (opts.emitNotification) opts.emitNotification({ type: "mr_review", title: `Review postado no MR !${id}`, body: `Comentário inline em ${file}:${line} via GitLab ${via}`, level: "success", mr_id: parseInt(id, 10), mr_url: mrUrl, provider });
                return { ok: true, provider, inline: true, file, line, via };
              }
            }
            ...
            if (noteRes.ok) {
              if (opts.emitNotification) opts.emitNotification({ type: "mr_review", title: `Review postado no MR !${id}`, body: `Comentário via GitLab ${via}`, level: "success", mr_id: parseInt(id, 10), mr_url: mrUrl, provider });
              return { ok: true, provider, message: `Comment posted via ${via}`, via };
            }
          }
        } catch (apiErr) {
          // API path failed → fall through to CLI
        }
      }
