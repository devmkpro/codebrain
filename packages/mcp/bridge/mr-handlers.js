...
function createMRHandlers(opts = {}) {
  return {
    ...
    async mrComment({ cwd, id, body, file, line }) {
      ...
      // ── Try OAuth bot token first (comment as bot, not as user) ──
      const getOAuthToken = opts.getOAuthToken;
      if (getOAuthToken) {
        const { provider, repo } = guard.gitInfo;
        const botToken = getOAuthToken(provider);
        if (botToken) {
          try {
            if (provider === "github") {
              // GitHub REST API: POST /repos/{owner}/{repo}/pulls/{number}/comments
              if (file && line) {
                // Inline review comment
                const prApiUrl = `https://api.github.com/repos/${repo}/pulls/${id}`;
                const prRes = await fetch(prApiUrl, {
                  headers: { Authorization: `Bearer ${botToken}`, Accept: "application/vnd.github.v3+json", "User-Agent": "Codebrain-ReviewBot" },
                });
            ...
            if (provider === "gitlab") {
              // GitLab API needs numeric project ID — resolve from repo slug
