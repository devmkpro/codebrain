...
export function AppFooter() {
  ...
  return (
    <footer className="h-9 shrink-0 relative">
      ...
      <div className="h-full border-t border-white/[0.04] bg-gradient-to-r from-[#0B0B0E] via-[#0D0D14] to-[#0B0B0E] flex items-center px-6 justify-between text-[10px] font-mono text-slate-600 uppercase tracking-[0.15em]">
        <div className="flex items-center gap-5">
          ...
          {folderName && <span className="text-slate-700 hover:text-slate-500 transition-colors cursor-default">{folderName}</span>}
          {activePanes > 0 && (
        ...
        <div className="flex gap-5 items-center">
          {appZoom !== 1 && <span className="text-yellow-400/50 bg-yellow-400/5 px-1.5 py-0.5 rounded">zoom {(appZoom * 100).toFixed(0)}%</span>}
          <span className={`${theme === 'light' ? 'text-sky-400/50 bg-sky-400/5' : 'text-slate-700'} px-1.5 py-0.5 rounded`}>
            {theme === 'light' ? 'light' : 'dark'}
          </span>
          <span className="text-slate-700">© 2026 CODEBRAIN</span>
        </div>
      </div>
    </footer>
  );
